function run_scale_maros_qoco()
    ...
    for k = 1:length(tests)
        ...
        obj = out.obj * oscale;
        obj
        row = {test_name,st{out.status},out.setup_time_sec,out.solve_time_sec,out.setup_time_sec + out.solve_time_sec, obj, out.iters};
        if out.status ~= 1
            row{1, 5} = inf;
        end
        results(k,:) = row;
        
        results.Properties.VariableNames = ["test","status","setup_time","solve_time","run_time","obj", "iters"];
        writetable(results, "scale_maros_qoco_results.csv");
    end
end
