function run_maros_debug()
%%%%%%% 问题，使用的初始化是xi=-s，导致Z的初始值很高（||Z^2||_inf ~ 1e9），LDL分解中L矩阵中出现inf（说明可能有接近零的不良值）
%%%%%%% 取reg=1e-6时发现可以避免inf （QPILOTNO,QFFFFF80）
%%%%%%%% 发现问题：初始的z值的数量级差异可能较大（从1e-k到1ek都会有）。但是检验过发现改进初始化（让s-rho *
%%%%%%%% xi接近0）并不能防止z值及其对应K矩阵的block的数量级更居中
