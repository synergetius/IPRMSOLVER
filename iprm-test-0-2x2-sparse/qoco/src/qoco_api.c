  ...
  IPRMSettings* settings)
{
  ...
  // 构造MtMPipe
  //printf("start construct pipe\n");
  QOCOCscMatrix* M = new_qoco_csc_matrix(solver->work->data->G);
  //for (int e = 0;e < M->nnz;e++){
  //  M->x[e] = 0;
  //}
  
  solver->work->mtmpipe = constructMtMPipe(M);
  //printf("mtm constructed\n");
  solver->work->apkbpipe = constructApkBPipe(
    solver->work->data->P,
    solver->work->mtmpipe->MtM
  );
  //printf("end construct pipe\n");

  iprm_allocate_kkt(solver->work);
  solver->work->kkt->PpkMtMtoKKT = 
    qoco_calloc(solver->work->apkbpipe->ApkB->nnz, sizeof(QOCOInt));
  //solver->work->kkt->PregtoKKT =
  //    qoco_calloc(solver->work->data->P->nnz, sizeof(QOCOInt));
  //solver->work->kkt->MtMtoKKT = 
  //    qoco_calloc(solver->work->mtmpipe->MtM->nnz, sizeof(QOCOInt));
  //solver->work->kkt->GTMGtoKKT =
  //    qoco_calloc(solver->work->kkt->GTMG->nnz, sizeof(QOCOInt));
  ...
  solver->work->kkt->rhs = qoco_malloc((n + p) * sizeof(QOCOFloat));
  //
  solver->work->kkt->xt = qoco_malloc((n + p) * sizeof(QOCOFloat));
  solver->work->kkt->xtbuff1 = qoco_malloc((n + p) * sizeof(QOCOFloat));
  solver->work->kkt->xtbuff2 = qoco_malloc((n + p) * sizeof(QOCOFloat));
  iprm_construct_kkt(solver);
  //print_K(solver->work->kkt->K);
  //printf("kkt constructed\n");
  ...
  solver->work->rhoy = qoco_malloc(m * sizeof(QOCOFloat));
  //printf("#a\n");
  solver->work->mu = 0;
  solver->work->rho = 0;
  ...
  //////
  solver->work->sbuff = qoco_malloc(m * sizeof(QOCOFloat));
  solver->work->xibuff = qoco_malloc(m * sizeof(QOCOFloat));
  ...
  //printf("#3.2\n");
  for (QOCOInt i = 0; i < solver->work->apkbpipe->ApkB->nnz; ++i) {
    solver->work->kkt->PpkMtMtoKKT[i] 
    = KtoPKPt[solver->work->kkt->PpkMtMtoKKT[i]];
  }
  //printf("#3.3\n");
  //printf("#4\n");
...
QOCOInt iprm_solve(IPRMSolver* solver)
{
  ...
  for (QOCOInt i = 1; i <= solver->settings->max_iters; ++i) {
    ...
    //printf("done update block\n");
    iprm_construct_kkt_rhs(work);
    //printf("done construct kkt rhs\n");
    iprm_kkt_solve(solver);// 得到Dx, Dt存储在kkt->xt中
    // 计算Ds
    //printf("done kkt solve\n");
    SpMv(data->G, work->kkt->xt, work->sbuff);
    for (int i = 0;i < data->m;i++){
      work->Ds[i] = 
        work->rhoy[i] * work->rhoy[i] * 
          work->sbuff[i] / work->mu 
        + work->rhoy[i] * work->rhoy[i] * 
    ...
    //printf("done compute Ds\n");
    // 计算Dxi
    copy_and_negate_arrayf(
      &work->kkt->psi[data->n + data->p + data->m], 
      work->Dxi,
      data->m
    );
    axpy(work->sbuff, work->Dxi, work->Dxi, -1.0, data->m);
    //printf("done compute Dxi\n");
    /*
    printf("Dmu = %.12e\n", work->Dmu);
    printf("||Dxi||_inf = %.12e\n", inf_norm(work->Dxi, data->m));
    printf("||Dx||_inf = %.12e\n", inf_norm(work->kkt->xt, data->n));
    printf("||Dt||_inf = %.12e\n", 
      inf_norm(&work->kkt->xt[data->n], data->p));
    printf("||Ds||_inf = %.12e\n", 
      inf_norm(work->Ds, data->m));
    printf("mu * ||Z^2||_inf = %.12e\n", 
      inf_norm(work->z, data->m) 
      * inf_norm(work->z, data->m) * work->mu);
    printf("mu = %.12e\n", work->mu);
    printf("rho = %.12e\n", work->rho);
    */
    ///////////////////// 修改进度到这里
...
{
  free_qoco_csc_matrix(solver->work->mtmpipe->M); // 它是新建的矩阵
  freeMtMPipe(solver->work->mtmpipe);
  freeApkBPipe(solver->work->apkbpipe);
  ...
  //free_qoco_csc_matrix(solver->work->kkt->GTMG); // 
  ...
  qoco_free(solver->work->kkt->xt);
  qoco_free(solver->work->kkt->xtbuff1);
  qoco_free(solver->work->kkt->xtbuff2);
  ...
  qoco_free(solver->work->rhoy);


  qoco_free(solver->work->xbuff);
  qoco_free(solver->work->tbuff);
  qoco_free(solver->work->sbuff);
  qoco_free(solver->work->xibuff);
  ...
  qoco_free(solver->work->kkt->PpkMtMtoKKT); //////
