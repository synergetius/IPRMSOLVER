...
TEST(linalg, new_qoco_csc_matrix_test)
{
  ...
  iprm_set_csc(A, m, n, Annz, Ax, Ap, Ai);

  IPRMCscMatrix* M = new_iprm_csc_matrix(A);
...
TEST(linalg, regularize_test1)
{
  ...
  IPRMCscMatrix* Pexp = (IPRMCscMatrix*)malloc(sizeof(IPRMCscMatrix));

  iprm_set_csc(P, n, n, Pnnz, Px, Pp, Pi);
  iprm_set_csc(Pexp, n, n, Pnnz, Px_exp, Pp, Pi);
...
TEST(linalg, regularize_test2)
{
  ...
  iprm_set_csc(P, n, n, Pnnz, Px, Pp, Pi);
  iprm_set_csc(Pexp, n, n, Pnnz_exp, Px_exp, Pp_exp, Pi_exp);

  IPRMCscMatrix* Pmalloc = new_iprm_csc_matrix(P);
  IPRMCscMatrix* Pexpmalloc = new_iprm_csc_matrix(Pexp);

  IPRMInt nz_added = regularize(Pmalloc, 1.0, nzadded_idx);
  ...
  free(P);
  free(Pexp);
  free_iprm_csc_matrix(Pmalloc);
  free_iprm_csc_matrix(Pexpmalloc);
}
...
TEST(linalg, regularize_test3)
{
  ...
  IPRMCscMatrix* Pexp = (IPRMCscMatrix*)malloc(sizeof(IPRMCscMatrix));
  iprm_set_csc(P, n, n, Pnnz, Px, Pp, Pi);
  iprm_set_csc(Pexp, n, n, Pnnzexp, Pxexp, Ppexp, Piexp);
...
TEST(linalg, ruiz_test)
{
  ...
  IPRMFloat Dexp[] = {1.0000, 0.8409, 0.6389, 0.7071, 0.7022, 0.6894};
  IPRMFloat Eexp[] = {1.0000, 0.7825};
  IPRMFloat Fexp[] = {1.0000, 1.1892, 1.5315, 1.4142, 1.4142, 1.4142};
  IPRMFloat kexp = 0.2480;
  IPRMFloat tol = 1e-4;
