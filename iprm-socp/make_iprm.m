...
function make_iprm(varargin)
% Matlab MEX makefile for IPRM.
%

%    MAKE_IPRM(VARARGIN) is a make file for IPRM solver. It
%    builds IPRM and its components from source.
%
...
%
%    'iprm': builds the IPRM solver using CMake
%
%    'iprm_mex': builds the IPRM mex interface and links it to the IPRM
%    library
%
...
elseif ( nargin == 1 && ismember('-verbose', varargin) )
    ...
    if(isempty(strfind(what, 'all'))        && ...
            isempty(strfind(what, 'iprm'))      && ...
            isempty(strfind(what, 'iprm_mex'))  && ...
...
%% Try to unlock any pre-existing version of iprm_mex

% this prevents compile errors if a user builds, runs iprm
% and then tries to recompile
if(mislocked('iprm_mex'))
    munlock('iprm_mex');
end
%%%%%%%%%%%%%%%%%%%%%%% 注意此处改动，可能在别处有呼应
...
lib_ext = '.a';
lib_name = sprintf('libiprmstatic%s', lib_ext);
qdldl_lib_name = 'libqdldl.a';


% Set iprm directory and iprm_build directory
current_dir = pwd;
[makefile_path,~,~] = fileparts(which('make_iprm.m'));
iprm_dir = fullfile(makefile_path, 'iprm');
iprm_build_dir = fullfile(iprm_dir, 'build');
qdldl_dir = fullfile(iprm_dir, 'lib', 'qdldl');
qdldl_types_dir = fullfile(iprm_build_dir, 'lib', 'qdldl');

% Include directory
inc_dir = [
    fullfile(sprintf(' -I%s', iprm_dir), 'include'), ...
...
%% IPRM Solver
if( any(strcmpi(what,'iprm')) || any(strcmpi(what,'all')) )
    fprintf('Compiling IPRM solver...');
    sprintf('%s %s ..', 'cmake', cmake_args)
    % Create build directory and go inside
    if exist(iprm_build_dir, 'dir')
        rmdir(iprm_build_dir, 's');
    end
    mkdir(iprm_build_dir);
    cd(iprm_build_dir);
    ...
    [status, output] = system(sprintf('%s %s', make_cmd, '--target iprmstatic'));
    if (status)
        fprintf('\n');
        disp(output);
        error('Error compiling IPRM');
    elseif(verbose)
        fprintf('\n');
        disp(output);
    end
    ...
    lib_origin = fullfile(iprm_build_dir, lib_name);
    copyfile(lib_origin, lib_name);
    
    % Copy libqdldl.a to current folder
    lib_origin = fullfile(iprm_build_dir, 'lib', 'qdldl', 'out', qdldl_lib_name);
...
%% iprmmex
if( any(strcmpi(what,'iprm_mex')) || any(strcmpi(what,'all')) )
    % Compile interface
    fprintf('Compiling and linking iprmmex...');
    
    % Compile command
    
    cmd = sprintf('%s %s %s %s %s iprm_mex.cpp %s', ...
...
if( any(strcmpi(what,'purge')) )
    fprintf('Cleaning IPRM build directory...');
    
    % Delete IPRM build directory
    if exist(iprm_build_dir, 'dir')
        rmdir(iprm_build_dir, 's');
    end
