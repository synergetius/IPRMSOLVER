...
void iprm_construct_kkt(IPRMSolver* solver)
{
  ...
  // Add non-negative orthant part of G^T and Block (5, 5)
  IPRMInt diag = 0;
...
void iprm_initialize(IPRMSolver* solver){
 ...
 /*  printf("||c||_inf =  %.12e\n", inf_norm(work->data->c, work->data->n));
  printf("||b||_inf =  %.12e\n", inf_norm(work->data->b, work->data->p));
  printf("||rhs||_inf =  %.12e\n", inf_norm(work->kkt->rhs, work->kkt->K->n));
  printf("||rhs_n||_inf =  %.12e\n", inf_norm(work->kkt->rhs, work->data->n));
  printf("||rhs_2nsoc||_inf =  %.12e\n", inf_norm(&work->kkt->rhs[work->data->n], work->data->nsoc + work->data->nsoc));
  printf("||rhs_p||_inf =  %.12e\n", inf_norm(&work->kkt->rhs[work->data->n + work->data->nsoc + work->data->nsoc], work->data->p));
  printf("||rhs_m||_inf =  %.12e\n", inf_norm(&work->kkt->rhs[work->data->n + work->data->nsoc + work->data->nsoc + work->data->p], work->data->m));
   */
...
void iprm_update_blocks(IPRMSolver* solver){
  ...
  for (IPRMInt c = 0; c < work->data->nsoc;++c){
    for (int i = idx;i < idx + work->data->q[c];i++){
      work->kkt->K->x[work->kkt->B26toKKT[i]] = -iprm_sqrt(2.0 / work->mu) * work->z[i];
    }
    idx += work->data->q[c];
  }
  //printf("done(2, 6)\n");
  ...
  for (IPRMInt c = 0; c < work->data->nsoc;++c){
    ...
    //printf("det = %.12e\n", det);
    work->kkt->K->x[work->kkt->B36toKKT[c]] = iprm_sqrt(2 * det / work->mu);
    idx += work->data->q[c];
  }
  //printf("done(3, 6)\n");
  // block (5, 5)
  for (IPRMInt i = 0;i < work->data->l;i++){
    work->kkt->K->x[work->kkt->B55toKKT[i]] = -work->z[i] * work->z[i] / work->mu - solver->settings->kkt_static_reg;
  }
  //printf("done(5, 5)\n");
  // block (6, 6)
  idx = work->data->l;
  for (IPRMInt c = 0; c < work->data->nsoc;++c){
    ...
    for (int i = idx;i < idx + work->data->q[c];i++){
      ...
      work->kkt->K->x[work->kkt->B66toKKT[i]] = -det / work->mu - solver->settings->kkt_static_reg; // det加正则化
    }
    idx += work->data->q[c];
  }
  //printf("done(6, 6)\n");
  //printf("||K||_inf = %.10e\n", inf_norm(work->kkt->K->x, work->kkt->K->nnz));
}
void iprm_construct_kkt_rhs(IPRMWorkspace* work)
{
  ...
  for (IPRMInt j = 0;j < work->data->l;j++){
    work->kkt->rhs[work->data->n + work->data->nsoc + work->data->nsoc + work->data->p + j] = -(
      work->rho * work->z[j] * work->z[j] * work->kkt->psi[work->data->n + work->data->p + j] / work->mu
      + work->kkt->psi[work->data->n + work->data->p + j]
      + work->Dmu * work->z[j] / work->mu
  ...
  for (IPRMInt c = 0;c < work->data->nsoc;c++){
    ...
    for (int j = idx;j < idx + work->data->q[c];j++){
      work->kkt->rhs[work->data->n + work->data->nsoc + work->data->nsoc + work->data->p + j] = -(
        (work->rho * det / work->mu + 1) * work->kkt->psi[work->data->n + work->data->p + j]
        + (2 * work->rho * zr + work->Dmu) * work->z[j] / work->mu 
        + work->kkt->psi[work->data->n + work->data->p + work->data->m + j]
      );
    }
    // 补上e_i项的系数（括号内减相当于去括号后的加）
    work->kkt->rhs[work->data->n + work->data->nsoc + work->data->nsoc + work->data->p + idx] +=
      2 * work->rho * det * work->kkt->psi[work->data->n + work->data->p + idx] / work->mu;
    idx += work->data->q[c];
  }
  printf("||rhs||_inf =  %.12e\n", inf_norm(work->kkt->rhs, work->kkt->K->n));
}
void iprm_kkt_multiply(IPRMSolver* solver, IPRMFloat* src, IPRMFloat* tar){
  ...
  for (int c = 0;c < data->nsoc;c++){
    ...
    tar[data->n + c] -= iprm_sqrt(2.0 / work->mu) * d;
    idx += data->q[c];
  }
  // Row 3
  ...
  for (int c = 0;c < data->nsoc;c++){
    ...
    tar[data->n + data->nsoc + c] += iprm_sqrt(2.0 * det / work->mu) * src[data->n + data->nsoc + data->nsoc + data->p + idx];
    idx += data->q[c];
  }
  // Row 4
  if (data->p > 0){
  ...
  if (data->m > 0){
    ...
    for (int i = 0;i < data->m;i++){
      tar[data->n + data->nsoc + data->nsoc + data->p + i] = work->sbuff[i];
    }
  }
  // Row 5
  for (int i = 0; i < data->l;i++){
    tar[data->n + data->nsoc + data->nsoc + data->p + i] -= 
      work->z[i] * work->z[i] * src[data->n + data->nsoc + data->nsoc + data->p + i] / work->mu;
  }
  // Row 6
  idx = data->l;
  for (int c = 0;c < data->nsoc;c++){
    ...
    axpy(&work->z[idx], 
      ...
      &tar[data->n + data->nsoc + data->nsoc + data->p + idx], 
      -iprm_sqrt(2.0 / work->mu) * src[data->n + c], data->q[c]);
    IPRMFloat det = soc_det(&work->z[idx], data->q[c]);
    // item 3
    tar[data->n + data->nsoc + data->nsoc + data->p + idx] += iprm_sqrt(2 * det / work->mu) * src[data->n + data->nsoc + c];
    // item 4
    axpy(&src[data->n + data->nsoc + data->nsoc + data->p + idx], 
      &tar[data->n + data->nsoc + data->nsoc + data->p + idx],
      &tar[data->n + data->nsoc + data->nsoc + data->p + idx],
      -det / work->mu, data->q[c]
    );
    idx += data->q[c];
  }
