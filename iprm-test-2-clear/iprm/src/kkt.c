...
void iprm_construct_kkt(IPRMSolver* solver)
{
  ...
  IPRMInt diag = 0;
  for (IPRMInt Gtcol = 0; Gtcol < work->data->m; ++Gtcol) {
    IPRMInt nzadded = 0;
...
void iprm_update_blocks(IPRMSolver* solver){
  ...
  for (IPRMInt j = 0; j < work->data->m;j++){
	...
	// w = iprm_max(w, iprm_abs(work->kkt->K->x[work->kkt->ZtoKKT[j]]));
	// u = iprm_min(u, iprm_abs(work->kkt->K->x[work->kkt->ZtoKKT[j]]));
  }
