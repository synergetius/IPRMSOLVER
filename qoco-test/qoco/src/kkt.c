...
{
// 由于K是对称阵，这里只构建了K的上三角部分
  QOCOWorkspace* work = solver->work;
  QOCOInt nz = 0;
  QOCOInt col = 0;
  // Add P block
  for (QOCOInt k = 0; k < work->data->P->nnz; ++k) {
    work->kkt->PregtoKKT[k] = nz; //看起来这个映射关系是用非零元素的序号来定义的
  ...
  for (QOCOInt Atcol = 0; Atcol < work->data->At->n; ++Atcol) {
    ...
    // Add -e * Id regularization. （这是A^T下面的分块的静态正则化）
...
{
	// 初始化步骤，求解方程组（9abc）的一组解再进行平移
  // Set Nesterov-Todd block in KKT matrix to -I. （见（23）式）
  ...
  // Construct rhs of KKT system. （残差）
  idx = 0;
...
void update_nt_block(QOCOSolver* solver)
{
  ...
  for (QOCOInt i = 0; i < solver->work->data->m; ++i) {
    solver->work->kkt->K->x[solver->work->kkt->ntdiag2kkt[i]] -=
        solver->settings->kkt_static_reg; // 此处使用的静态正则化常数与对P用的常数是同一个，根据论文这里是减去
  }
}
...
void compute_kkt_residual(QOCOSolver* solver)
{
  ...
  kkt_multiply(solver, work->kkt->xyzbuff1, work->kkt->kktres); // 计算残差有关上一轮迭代的解的部分
  ...
  for (idx = 0; idx < work->data->n; ++idx) {
    work->kkt->kktres[idx] =
        work->kkt->kktres[idx] +
        (work->data->c[idx] - solver->settings->kkt_static_reg * work->x[idx]); //减掉静态正则化的影响，这意味着代入残差第一个行分块的P是原始的而非静态正则化后的P
  }
  ...
  for (QOCOInt i = 0; i < work->data->n; ++i) {
    regularization_correction +=
        solver->settings->kkt_static_reg * work->x[i] * work->x[i]; // 同上，这里也是减掉静态正则化的影响，使得此处算出的目标函数值是基于原始的P矩阵的
  }
  obj += 0.5 *
...
void predictor_corrector(QOCOSolver* solver)
{
  ...
  // Compute Dsaff. Dsaff = W' * (-lambda - W * Dzaff). （应该是利用(18d)式导出的）
  QOCOFloat* Dzaff = &work->kkt->xyz[work->data->n + work->data->p];
